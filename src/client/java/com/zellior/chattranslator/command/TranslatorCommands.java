...
"af",
				...
				.then(literal("chat")
						.then(argument("language", StringArgumentType.word())
								...
								.executes(context -> {
									...
									config.save();
									context.getSource().sendFeedback(Component.literal("Chat translation target set to " + language + ".").withStyle(ChatFormatting.GREEN));
									return 1;
								})))
				.then(literal("off")
						.executes(context -> {
							...
							config.save();
							context.getSource().sendFeedback(Component.literal("Chat translation disabled.").withStyle(ChatFormatting.YELLOW));
							return 1;
						}))
				.then(literal("on")
						.executes(context -> {
							config.setChatTranslationEnabled(true);
							config.save();
							context.getSource().sendFeedback(Component.literal("Chat translation enabled for " + config.getIncomingTargetLanguage() + ".").withStyle(ChatFormatting.GREEN));
							return 1;
						}))
				.then(literal("msg")
						.then(argument("language", StringArgumentType.word())
								...
								.then(argument("message", StringArgumentType.greedyString())
										.executes(context -> {
											...
											context.getSource().sendFeedback(Component.literal("Translating message...").withStyle(ChatFormatting.GRAY));
											return 1;
										})))));
	}
	...
	private static void translateAndSend(String language, String message, TranslationService translationService) {
		...
		future.thenAccept(result -> Minecraft.getInstance().execute(() -> {
			...
			client.getConnection().sendChat(translatedMessage);
			client.player.sendSystemMessage(Component.literal("[Translator] Sent: ").withStyle(ChatFormatting.AQUA)
					.append(Component.literal(translatedMessage).withStyle(ChatFormatting.WHITE)));

			if (!translatedMessage.equals(result.translatedText())) {
				client.player.sendSystemMessage(Component.literal("[Translator] Message was shortened to fit Minecraft's 256 character chat limit.").withStyle(ChatFormatting.YELLOW));
			}
		})).exceptionally(error -> {
			ChatTranslatorClient.sendLocalStatus("Message translation failed: " + error.getMessage(), ChatFormatting.RED);
			return null;
		});
	}
